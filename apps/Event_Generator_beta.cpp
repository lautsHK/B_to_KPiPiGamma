...
int main ( int argc, char* argv[] )
{
   ...
   */
   ...
   //Find out the maximum value of the square of matrix elements
   double max_ME2[2] = {0., 0.}; //The maximum value of the matrix elements of right-handed and left-handed, where 0 is RH.
   int test_numbers[2] = {0, 0};

   for(int side=0; side<2; side++)
   {
      while( test_numbers[side] < 5000 )
      {
         double s_inv;
         ...
         else if(generation_mode == 0)
         {
            s_inv = s_max;
         }
         else
         {
            std::cout << "Error message : you should not be here\n.";
         }

         //double s23 = generate.BreitWigner( mass_Kst, width_Kst, s23_min, s23_max ); //This generation is NOT okay to used for generate MC-samples unless the Hit-or-miss function is modified. Here we just want to find out the maximum value.
         ...
         if( (s23 >= s23_Dmin) && (s23 <= s23_Dmax) )
         {
            ...
            if(side == 0)
            { 
               value_ME2 = MEs.ME2_RH(s_inv, s13, s23, cos_theta, phi, K_charge, T1K1270, T1K1400, T1K1410, T1K1430, T1K1680); 
            }
            else if(side == 1)
            {
               value_ME2 = MEs.ME2_LH(s_inv, s13, s23, cos_theta, phi, K_charge, T1K1270, T1K1400, T1K1410, T1K1430, T1K1680); 
            }
            else
            {
               std::cout << "Error message : you should not be here\n.";
            ...
            if (value_ME2 > max_ME2[side] )
            {
               max_ME2[side] = value_ME2;
            }       

            if ( remainder(test_numbers[side], 1000) == 0 )
            {
               std::cout << "Checking the maximum value of the Matrix Element : Loop " << test_numbers[side] << " .\n"; //Print message for every 1000 generations
            }
   ...
   std::cout << "The maximum value of the square of Right-handed Matrix Element is " << max_ME2[0] << " .\n";
   std::cout << "The maximum value of the square of Left-handed Matrix Element is " << max_ME2[1] << " .\n";
   ...
   {
      "./../output/" + prifix + "_photon.dat",
      "./../output/" + prifix + "_P1.dat",
      "./../output/" + prifix + "_P2.dat",
      "./../output/" + prifix + "_P3.dat",
   };
         ...
         if( (s23 >= s23_Dmin) && (s23 <= s23_Dmax) )
         {
            ...
            { 
               ...
               */ 
               ...
               /*
               double photon_E_Lab = (pow(mass_B, 2.0) - s_inv) / (2. * mass_B ); //Lab-frame
               if( std::abs( photon_Lab[0] - photon_E_Lab ) > 1e-8 )
               {
                  std::cout << "The value of the boosted photon is " << photon_Lab[0] << " and " << photon_E_Lab << " .\n";
               } 
               */
               ...
               /* The energy and momentum of a photon should be the same when c==1
               if( std::abs( photon_Lab[0] - photon[3] ) > 1e-8 )
               {
                  std::cout << "The value of the boosted photon is " << photon_Lab[0] << " and " << photon[3] << " .\n";
               } 
               */
